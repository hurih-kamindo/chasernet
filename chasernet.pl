...
use vars qw/%chasernet %CONFIGFILE %COUNTERS %db_extensions/;
...
config_init();
setup_dirs();
require "$CONFIGFILE{'PLUGINDIR'}/chasernet_core.plugin";
...
sub config_init {
    ...
    # Guess chasernet current directory
    my $NIKTODIR = abs_path($0);
    chomp($NIKTODIR);
    $NIKTODIR =~ s#[\\/]chasernet.pl$##;
    ...
    push(@CF,"$NIKTODIR/chasernet.conf.default");
    push(@CF,"/etc/chasernet.conf");
    push(@CF,"$home/chasernet.conf");
    push(@CF,"$NIKTODIR/chasernet.conf");
    push(@CF,"chasernet.conf");
...
sub setup_dirs {
    ...
    chomp($CURRENTDIR);
    $CURRENTDIR =~ s#[\\/]chasernet.pl$##;
    $CURRENTDIR = "." if $CURRENTDIR =~ /^chasernet.pl$/;
    ...
    unless (defined $CONFIGFILE{'EXECDIR'}) {
        if (-d "$ENV{'PWD'}/plugins") {
            ...
            print STDERR "Could not work out the chasernet EXECDIR, try setting it in chasernet.conf\n";
            exit;
        }
    }
...
sub check_config_defined {
    ...
    if (!defined $CONFIGFILE{$item}) {
        print STDERR
          "- Warning: $item is not defined in chasernet configuration, setting to \"$default\"\n";
